<template>
  <div class="todo">
    ...
    <div class="top-input">
      <input type="text" placeholder="請輸入新事項" v-model.trim="newInput" />
      ...
      <li v-for="item of list" :key="item.tId">
        ...
        <template v-if="edit !== item.tId">
          <div class="input">
            ...
            <input type="checkbox" v-model="item.todo.done" @change="checkTodo({tId: item.tId, done: item.todo.done})" />{{ item.todo.content }}
          </div>
          <div class="button">
...
export default {
  ...
  computed: {
    ...
    allList() {
      return this.$store.getters.filterList('all');
    }
  },
  methods: {
    ...
    upRecord(index) {
      ...
      // TODO 找到 filter 後（Active | Done）上面那筆資料的 tId 後，利用 tId 找到該筆資料的順序之後做交換
      // 利用 tId 找該筆資料在陣列裡的位置
      // if (index <= 0) {
      //   return 
      // }
      ...
      // 找到被點擊的資料的陣列位置
      const currentIndex = this.list.findIndex((item) => {
        return item.tId === index
      })
      console.log(this.list[currentIndex - 1])
      // 找到被點擊的資料的上面那筆資料的陣列位置
      // this.allList 改成實際的資料陣列
      const beforeIndex = this.allList.findIndex((item) => {
        return item.tId === this.list[currentIndex - 1].tId
      })
